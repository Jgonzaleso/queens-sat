...
int choose_P(const bitmask* d0, int ns, int max_combos, int P_max) {
    int P = 0; long long combos = 1;
    for (int i = 0; i < ns && P < P_max; i++) {
        long long nc = combos * (long long)pop_bm(d0[i]);
...
static bool greedy_complete(bitmask* dr, const int* rows, int nr, int N, int* out_cols) {
    if (nr == 0) return true;
    bitmask cur[MAXN]; memcpy(cur, dr, nr*sizeof(bitmask));
    int rem[MAXN]; memcpy(rem, rows, nr*sizeof(int));
    int nrem = nr;
    for (int step = 0; step < nr; step++) {
        int best = 0;
        for (int i = 1; i < nrem; i++) if (pop_bm(cur[i]) < pop_bm(cur[best])) best = i;
        if (!cur[best]) return false;
        int col = ctz_bm(cur[best]);
        int row = rem[best];
        out_cols[row] = col;
        int nnr = 0; bitmask nc[MAXN]; int nr2[MAXN]; bool ok = true;
        ...
        if (!ok) return false;
        if (nnr > 0 && ac3_bits(nc, nr2, nnr, N)) return false;
        memcpy(cur, nc, nnr*sizeof(bitmask));
...
static PResult pivot_enum(bitmask* doms, const int* rows, int ns, int N,
                           int budget, int depth) {
    if (ns == 0) return SAT_POSSIBLE;
    int order[MAXN]; for (int i=0;i<ns;i++) order[i]=i;
    std::sort(order, order+ns, [&](int a,int b){ return pop_bm(doms[a])<pop_bm(doms[b]); });
    bitmask sd[MAXN]; int sr[MAXN];
    ...
    for (int pi=0;pi<P;pi++) {
        int sz=0; bitmask tmp=sd[pi];
        while(tmp) { int c=ctz_bm(tmp); tmp&=tmp-1; pv[pi][sz++]=c; }
        psz[pi]=sz;
    }
    ...
    if (g_mcv_enabled) {
        for (int pi=0;pi<P;pi++) {
            ...
            for (int ci=0;ci<psz[pi];ci++) {
                ...
                for (int i=0;i<nr;i++)
                    elim += pop_bm(sd[P+i] & attack_mask(sr[pi],c,rest[i],N));
                imp[ci]=elim;
            }
    ...
    for (long long idx=0; idx<total; idx++) {
        ...
        if (depth == 0) {
            ...
            std::partial_sort(ro,ro+nsac,ro+nr,[&](int a,int b){ return pop_bm(dr[a])<pop_bm(dr[b]); });
            int sr2[MAXN]; bitmask sd2[MAXN];
            for (int i=0;i<nsac;i++) { sr2[i]=rest[ro[i]]; sd2[i]=dr[ro[i]]; }
            bitmask d2[MAXN]; memcpy(d2, dr, nr*sizeof(bitmask));
...
static PResult pivot_enum_solve(bitmask* doms, const int* rows, int ns, int N,
...
PResult pipeline_solve(const int* qr, const int* qc, int k, int N, int* sol_cols) {
    ...
    for (int r=0;r<N;r++) {
        ...
        if (!doms0[ns]) return UNSAT_DET;
        singles[ns++] = r;
        sol_cols[r] = -1;
    }
    { bitmask tmp[MAXN]; memcpy(tmp,doms0,ns*sizeof(bitmask));
      if (propagate_all(tmp,singles,ns,N)) return UNSAT_DET;
      memcpy(doms0,tmp,ns*sizeof(bitmask)); }
    if (ns==0) return SAT_POSSIBLE;
    if (ns==1) { sol_cols[singles[0]]=ctz_bm(doms0[0]); return SAT_POSSIBLE; }
...
static CMRResult cmr_analyze(const int* qr, const int* qc, int K, int N) {
    ...
    for (int i=0;i<ns;i++) {
        int sz=pop_bm(doms0[i]); sum_dom+=sz; min_dom=std::min(min_dom,sz);
    }
    res.min_dom=min_dom;
